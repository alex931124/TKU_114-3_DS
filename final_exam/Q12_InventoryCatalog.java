...
public class Q12_InventoryCatalog {
    ...
    public Q12_Product binarySearchById(
        ...
        if (sortedProducts == null || sortedProducts.length == 0 || id == null || id.trim().isEmpty()) {
            return null;
        }
        ...
        while (low <= high) {
            int catalogTraceD19A = low + (high - low) / 2;
            int cmp = sortedProducts[catalogTraceD19A].getId().compareToIgnoreCase(targetId);

            if (cmp == 0) {
                return sortedProducts[catalogTraceD19A];
            } else if (cmp < 0) {
                low = catalogTraceD19A + 1;
            } else {
                high = catalogTraceD19A - 1;
            }
        }
